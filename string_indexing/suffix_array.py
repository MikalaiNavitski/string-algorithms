...
def sadakaneSort(I, V, S, start, end, k, group_index):
  n = len(I)
  if start >= end:
    if start > end:
      S.pop(group_index)
  ...
  sadakaneSort(I, V, S, equal, end, k, group_index + 1 + val)
  sadakaneSort(I, V, S, start, less - 1, k, group_index)


def sadakane(text, n):
  '''Computes suffix array using Sadakane's "Fast Algorithm for Making Suffx Arrays and for Burrows-Wheeler Transformation" algorithm'''
  text += '$'
  ...
  while k <= n and S[0] != -(n + 1):
    ...
    while i >= 0:
      ...
      if S[i] < 0 :
        ...
        saveGroupIndex = group_index
        group_index -= S[i]
        sadakaneSort(I, V, S, saveGroupIndex - S[i] + 1, saveGroupIndex, k, i)
        i -= 1
    k *= 2
